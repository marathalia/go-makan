...
function cuisineFromTypes(types = [], name = "") {
  ...
  if (/soup|noodle|ramen|pho|noodle/.test(typeText)) matches.push("Noodles");
...
function mapGooglePlace(place, origin, searchHints = []) {
  ...
  const priceLevel = moneyFromPriceLevel(place.price_level);
...
function installPlacesApi(server, apiKey) {
  server.middlewares.use("/api/location-suggest", async (req, res) => {
    if (!apiKey) {
      sendJson(res, 200, { source: "fallback", suggestions: [] });
      return;
    }

    try {
      const requestUrl = new URL(req.url, "http://localhost");
      const input = requestUrl.searchParams.get("q")?.trim();

      if (!input || input.length < 2) {
        sendJson(res, 200, { source: "google", suggestions: [] });
        return;
      }
  ...
  server.middlewares.use("/api/location-detail", async (req, res) => {
    if (!apiKey) {
      sendJson(res, 503, { error: "Missing GOOGLE_MAPS_API_KEY" });
      return;
    }
  ...
  server.middlewares.use("/api/places", async (req, res) => {
    if (!apiKey) {
      sendJson(res, 200, { source: "fallback", places: [] });
      return;
    }
    ...
    try {
      ...
      const radius = Math.min(Number(requestUrl.searchParams.get("radius") || 1500), 5000);
      const mood = requestUrl.searchParams.get("mood") || "Any";
      const cuisines = (requestUrl.searchParams.get("cuisines") || "").split(",").filter(Boolean);
      const dietary = requestUrl.searchParams.get("dietary") || "No restriction";
      const budget = requestUrl.searchParams.get("budget") || "Any";
      const spice = requestUrl.searchParams.get("spice") || "Either";
      ...
      const payloads = await Promise.all(requestUrls.map((item) => fetchJson(item.url).then((payload) => ({ ...item, payload }))));
      ...
      sendJson(res, 200, { source: "google", places, keywords });
    } catch (error) {
  ...
  server.middlewares.use("/api/place-photo", (req, res) => {
    ...
    const maxWidth = requestUrl.searchParams.get("maxWidth") || "900";

    if (!reference) {
      res.statusCode = 400;
      res.end("reference is required");
      return;
    }
